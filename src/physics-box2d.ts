...
export type Box2DLoader = () => Promise<Box2DModule>;
...
export class Box2dPhysics implements IPhysics {
  ...
  /**
   * @param loader Box2D 모듈을 만든다. 환경마다 wasm을 찾는 방법이 달라서 밖에서 받는다.
   *   모든 참가자가 같은 바이너리(SIMD 미사용 빌드)를 써야 결과가 같다.
   */
  constructor(private loader: Box2DLoader) {}

  async init(): Promise<void> {
    this.Box2D = await this.loader();
    this.gravity = new this.Box2D.b2Vec2(0, 10);
    this.world = new this.Box2D.b2World(this.gravity);
  }

  /**
   * 월드를 새로 만든다. 같은 월드에서 body를 만들고 지우기를 반복하면 브로드페이즈 내부 id 재사용 순서가
   * 이력에 따라 달라져 접촉 처리 순서가 바뀔 수 있다. 라운드마다 새 월드로 시작해야 참가자 전원이
   * 이력과 무관하게 같은 결과를 얻는다.
   */
  resetWorld(): void {
