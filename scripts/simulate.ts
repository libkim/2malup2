 ...
 *
 * 시뮬레이션 난수(utils/random)를 시드 고정으로 쓰므로 같은 조건이면 같은 궤적이 나온다.
 */
...
const loadBox2D: Box2DLoader = () => {
  const factory = require('../node_modules/box2d-wasm/dist/umd/Box2D.js');
  const wasmBinary = fs.readFileSync(new URL('../node_modules/box2d-wasm/dist/umd/Box2D.wasm', import.meta.url));
...
async function simulate({ mapIdx, n, timeScale, stuckLenSq, seed }: Options): Promise<Result> {
  ...
  const p = new Box2dPhysics(loadBox2D);
  await p.init();
  p.createStage(stage);
